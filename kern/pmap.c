...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
    // -KERNBASE = 2^32 - KERNBASE in 2's complement
...
mem_init_mp(void)
{
    ...
    int i;
	for (i = 0; i < NCPU; i++) {
		int kstacktop_i = KSTACKTOP - i * (KSTKSIZE + KSTKGAP) - KSTKSIZE;
...
page_init(void)
{
    ...
    // Memory 0 ~ 640KB can be used (except for the first page)
	size_t i;
    ...
    // Memory 640kB ~ 1MB cannot be used (it is for BIOS and Video display)

    // Memory above 1MB can be used (except for the part used by kernel)
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
    ...
    // increment pp_ref before remove, otherwise, it may possible that
    // pp_ref goes to 0, and page_free it (which will be wrong for the corner-case)
	pp->pp_ref++;
	if (*pte & PTE_P) {
