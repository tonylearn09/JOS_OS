...
trap_init(void)
{
    ...
    // For JOS, we use interrupt gate as kernel cannot be interrupted (istrap=0)
    //
    ...
    // T_BRKPT DPL 3 (we abuse it in JOS)
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if (curenv->env_pgfault_upcall) {
		...
		if (tf->tf_esp >= UXSTACKTOP-PGSIZE && tf->tf_esp <= UXSTACKTOP-1) {
            // 4 for pushing an empty 32-bit word
			utf = (struct UTrapframe *)(tf->tf_esp - sizeof(struct UTrapframe) - 4);
		} else {
			utf = (struct UTrapframe *)(UXSTACKTOP - sizeof(struct UTrapframe));
		}

        // Check if we are in the memory between exception stack and normal stack
        // If it is, destory the environment
        ...
        // Set to use exception stack and run env_pgfault_upcall
